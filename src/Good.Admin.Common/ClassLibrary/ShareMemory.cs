...
namespace Good.Admin.Common.ClassLibrary
{
    ...
    public class ShareMenmory
    {
        ...
        public void Init(string strName, long lngSize)
        {
            ...
            if (strName.Length > 0)
            {
                ...
                m_hSharedMemoryFile = CreateFileMapping(INVALID_HANDLE_VALUE, IntPtr.Zero, PAGE_READWRITE, 0, (uint)lngSize, strName);
    ...
    public class ShareMenmory<T>
    {
        ...
        public ShareMenmory(string strName, long lngSize)
        {
            ...
            var method = t.GetMethod("getLength");
        ...
        public void Init(string strName, long lngSize)
        {
            ...
            {
                //创建内存共享体(INVALID_HANDLE_VALUE)
                m_hSharedMemoryFile = ShareMenmory.CreateFileMapping(INVALID_HANDLE_VALUE, IntPtr.Zero, PAGE_READWRITE, 0, (uint)lngSize, strName);
                if (m_hSharedMemoryFile == IntPtr.Zero)
                {
                    m_bAlreadyExist = false;
                    m_bInit = false;
        ...
        {
            var offset = 1 + index * (entityLength + 1) + 8;
        ...
        {
            var offset = 1 + index * (entityLength + 1) + entityLength - 2;
        ...
        {
            var offset = 1 + index * (entityLength + 1) + entityLength - 2;
            Write(state.ToString().ToByteArray(), offset);
        }
        ...
        {
            var offset = index * entityLength;
            Write(t.EntityToJson().ToByteArray(), offset);
        }
